...
macro_rules! profile_test {
    ...
    (routes: [$($route:expr),+], budget: $budget:expr, with_client: $with_client:expr, with_metrics: $with_metrics:expr) => {
        ...
        let client = client::http1(proxy.outbound, host);
...
fn retry_uses_budget() {
    profile_test! {
        ...
        with_metrics: |metrics: client::Client| {
            ...
            assert_eventually_contains!(
                metrics.get("/metrics"),
                "route_actual_retry_skipped_total{direction=\"outbound\",dst=\"profiles.test.svc.cluster.local:80\",skipped=\"timeout\"} 0"
            );
        }
    }
}
...
#[test]
fn does_not_retry_if_past_timeout() {
    profile_test! {
        routes: [
            controller::route()
                .request_any()
        ...
        budget: Some(controller::retry_budget(Duration::from_secs(10), 0.1, 1)),
        with_client: |client: client::Client| {
            let res = client.request(&mut client.request_builder("/0.5/sleep"));
            assert_eq!(res.status(), 533);
        },
        with_metrics: |metrics: client::Client| {
            assert_eventually_contains!(
                metrics.get("/metrics"),
                "route_actual_retry_skipped_total{direction=\"outbound\",dst=\"profiles.test.svc.cluster.local:80\",skipped=\"budget\"} 0"
            );
            assert_eventually_contains!(
                metrics.get("/metrics"),
                "route_actual_retry_skipped_total{direction=\"outbound\",dst=\"profiles.test.svc.cluster.local:80\",skipped=\"timeout\"} 1"
            );
        }
    }
}
